...
func (adbDevice AdbDevice) ForWardPort(remote interface{}) int {
	if port, ok := remote.(int); ok {
		remote = fmt.Sprintf("tcp:%d", port)
	}
	for _, f := range adbDevice.ForwardList() {
		if f.Serial == adbDevice.Serial && f.Remote == remote && strings.HasPrefix(f.Local, "tcp:") {
			i, err := strconv.ParseInt(f.Local[len("tpc:"):], 10, 32)
			if err != nil {
				panic(err)
			}
			return int(i)
	...
	adbDevice.ForWard(fmt.Sprintf("tcp:%d", localPort), remote.(string), false)
	return localPort
}
